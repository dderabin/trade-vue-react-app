...
export const BuySellForm = (props) => {
  ...
  const handleSubmitOrder = () => {
    ...
    const calculatedAmount = activeCrypto === props.tradingSymbol.from ? parseFloat(amount) : parseFloat(amount) / props.chartInfo.price;
  ...
  const handleUpdateOrder = () => {
    dispatch(AppActions.signalUpdateAction({      
      ...
      stopLoss: parseFloat(stopLoss),
      targets: updateProfit.map(item => ({price: parseFloat(item.price) || 0, amount: parseFloat(item.amount) || 0}))
    }))
  }
